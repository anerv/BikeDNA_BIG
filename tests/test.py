...
segments = gpd.GeoDataFrame(
    data={"seg_id_col": seg_ids, "edge_id_col": feature_ids}, geometry=lines
)

matches_ix = [44, 55, 66, 77, 88, 122]
matched_id = [4, 5, 6, 7, 8, 12]
segment_matches = pd.DataFrame(
    data={"matches_ix": matches_ix, "matches_id": matched_id}
)

matched, undecided = mf.summarize_feature_matches(
    segments, segment_matches, "seg_id_col", "edge_id_col", osm=True
)
...
segment_matches = pd.DataFrame(
    data={"matches_ix": matches_ix, "matches_id": matched_id, "road_type": road_type}
)

test_summarize = mf._summarize_attribute_matches(
    segments,
    segment_matches,
    attr="road_type",
...
test_osm = mf.update_osm(
    segments, osm, segment_matches, "road_type", "edge_id_col", "seg_id_col"
)
